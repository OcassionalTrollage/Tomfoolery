...
local function MWZVO_fake_script() -- Main.KSClient 
...
coroutine.wrap(MWZVO_fake_script)()
local function OZTL_fake_script() -- Main.smoothdrag 
	local script = Instance.new('LocalScript', Main)
...
coroutine.wrap(OZTL_fake_script)()
local function VWVG_fake_script() -- Main.rainbowClient 
	local script = Instance.new('LocalScript', Main)

	while task.wait() do
		for _,v in next, script.Parent:GetChildren() do
			if v:FindFirstChild('TextLabel') then
				v.TextLabel.TextColor3 = Color3.fromHSV(tick()%5/5, 1, 1)
			end
		end
	end
end
coroutine.wrap(VWVG_fake_script)()
